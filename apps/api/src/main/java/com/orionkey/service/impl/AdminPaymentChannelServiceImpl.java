...
public class AdminPaymentChannelServiceImpl implements AdminPaymentChannelService {
    ...
    public List<?> listChannels() {
        return paymentChannelRepository.findByIsDeletedOrderBySortOrderAsc(0).stream()
                .filter(channel -> channel.getProviderType() != null
                        && SUPPORTED_PROVIDER_TYPES.contains(channel.getProviderType().toLowerCase(Locale.ROOT)))
    ...
    public void createChannel(Map<String, Object> req) {
        ...
        if (req.containsKey("provider_type")) {
            channel.setProviderType(normalizeProviderType((String) req.get("provider_type")));
        }
    ...
    private static final Set<String> SENSITIVE_KEYS = Set.of(
            "api_token", "key", "secret", "password", "private_key", "merchant_private_key"
    );
    ...
    private String normalizeProviderType(String providerType) {
        if (providerType == null || providerType.isBlank()) {
            throw new BusinessException(ErrorCode.BAD_REQUEST, "支付渠道类型不能为空");
        }
        String normalized = providerType.toLowerCase(Locale.ROOT);
        if (!SUPPORTED_PROVIDER_TYPES.contains(normalized)) {
            throw new BusinessException(ErrorCode.BAD_REQUEST, "不支持的支付渠道类型");
        }
        return normalized;
    }
}
