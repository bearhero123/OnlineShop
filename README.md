...
项目提供 `docker-compose.local.yml` 用于本地一键启动 `web + api + postgres` 三个容器。
...
```bash
docker compose --env-file .env.docker -f docker-compose.local.yml logs -f
...
> 本地 Docker 默认使用 `.env.docker` 中的开发配置。数据库数据保存在命名卷 `postgres_data`，上传文件保存在仓库根目录 `./uploads`。
