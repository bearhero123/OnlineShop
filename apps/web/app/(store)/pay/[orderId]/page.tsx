...
export default function PaymentPage({ params }: { params: Promise<{ orderId: string }> }) {
  ...
  const canOpenCashier = qrcodeUrl.startsWith("http://") || qrcodeUrl.startsWith("https://")
    ...
    async function fetchOrderInfo() {
      try {
        ...
        if (!qrFromParam) {
          ...
          } else if (!result.pay_url && result.payment_url) {
            setQrcodeUrl(result.payment_url)
          }
        }
        if (!payurlFromParam && result.pay_url) {
          setPayUrlH5(result.pay_url)
        }
  ...
  useEffect(() => {
    if (!isMobile || !payUrlH5 || status !== "PENDING" || isWechatMobile) return
  ...
  }, [isMobile, payUrlH5, status, orderId, isWechatMobile])
