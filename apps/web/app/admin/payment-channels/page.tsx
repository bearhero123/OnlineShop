...
const PROVIDER_OPTIONS: ProviderOption[] = [
  {
    type: "epay",
    name: "易支付（聚合支付）",
    description: "兼容旧版 MD5 接口和新版 RSA SDK，可用于支付宝/微信聚合支付",
    channels: [
    ...
    configFields: [
      ...
      { key: "key", label: "商户密钥 (Key，可选，旧版 MD5)", placeholder: "旧版接口使用的 MD5 密钥", type: "password" },
      { key: "merchant_private_key", label: "商户私钥 (新版 RSA，可选)", placeholder: "支付.md 中的商户私钥", type: "password" },
      { key: "platform_public_key", label: "平台公钥 (新版 RSA，可选)", placeholder: "支付.md 中的平台公钥" },
      { key: "create_method", label: "创建方式 (可选)", placeholder: "留空默认 jump，也可填写 web / jump" },
    ],
  },
  {
    type: "native_alipay",
        ...
        <div className="flex flex-col gap-4 p-6">
          ...
          {currentProvider && currentProvider.configFields.length > 0 && formData.channel_code && (
            <div className="flex flex-col gap-3 rounded-lg border border-border bg-muted/20 p-4">
              ...
              {currentProvider.type === "epay" && (
                <p className="text-xs leading-5 text-muted-foreground">
                  旧版网关填写 `key` 即可；如果使用你提供的新版 SDK，请填写 `merchant_private_key` 和 `platform_public_key`，`key` 可留空。
                </p>
              )}
