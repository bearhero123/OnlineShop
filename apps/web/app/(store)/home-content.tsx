...
export function HomeContent({ products, categories, siteSlogan }: HomeContentProps) {
  const { t } = useLocale()
  const { config } = useSiteConfig()
  const { searchQuery, sortBy, inStockOnly, priceMin, priceMax } = useSearch()

  const [selectedCategory, setSelectedCategory] = useState<string | null>(null)
  const homepageAnnouncement = config?.announcement?.trim() ?? ""
      ...
      <section className="relative overflow-hidden rounded-xl border border-border/60 bg-card px-5 py-5 sm:px-8 sm:py-6">
        ...
        <div className="relative">
          ...
          {homepageAnnouncement ? (
            <div className="mt-4 rounded-2xl border border-amber-500/25 bg-[linear-gradient(135deg,rgba(251,191,36,0.12),rgba(249,115,22,0.08))] p-4 shadow-sm">
              <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-amber-500/20 bg-background/80 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-amber-700 dark:border-amber-400/20 dark:bg-background/60 dark:text-amber-300">
